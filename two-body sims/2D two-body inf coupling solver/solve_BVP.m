function [output_BVP, nmaxErr] = solve_BVP(x0,p0,xf,tf,params)
...
while i <= params.nmax 
    ...
    % Solve IVP
    output_IVP = solve_IVP(x0,p0,tf,params,1);
...
%     output_IVP.eta = xf-output_IVP.x(end,:);
    ...
    %[~,~,V] = svd(jacobian);
    %dp0 = V(:,end);
    dp0 = -jacobian(:,1:3)\output_IVP.eta';
    dp0= [dp0;dp0];
...
while newerr > (1-params.decrease_param)*err
    ...
    output_IVP = solve_IVP(x0,newp0,tf,params,1);
